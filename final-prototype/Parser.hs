...
parseExpression = do
  expression <- chainr1 parseEElement (parseCons (ENode "_"))
  spaces
  return expression
...
validExpression variableNames functionFrame (ENil name) = (ENil name)
validExpression variableNames functionFrame (ENode name leftE rightE) =
  let
  ...
  in
    ENode name validLeftE validRightE
