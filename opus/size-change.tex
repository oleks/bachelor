...
our programs. Indeed because by the semantics of \D{}, node construction and
...
of control, that is, arcs.

\begin{definition} A control flow graph for a given program $r=\left\langle F,x
\right\rangle$, with the set of clauses $$C= \left\{ c \mid f= \left\langle v,C
\right\rangle \in F \wedge c\in C \right\},$$ is a graph with the set of nodes
$C$, and the set of directed edges 

\begin{align*}
E=\{ \left\langle c_1,c_2,x\right\rangle \mid\ 
& c_1 = \left\langle v_1, p_1, x_1 \right\rangle \in C \wedge\\
& c_2 = \left\langle v_2, \_, \_ \right\rangle \in C \wedge\\
& x\in\mathbb{X} \wedge \left\langle v_2, x \right\rangle \Subset x_1\}
.\end{align*}

\end{definition}

Control flow graphs of this sort can more intuitively be referred to as
\emph{call graphs}.

We use a triplet to ensure that calls with different arguments get different edges.
