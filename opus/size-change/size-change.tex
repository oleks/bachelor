...
\wedge&c_s = \left\langle \_, p_s, \_ \right\rangle \in C \\
\wedge&c_t = \left\langle \_, p_t, \_ \right\rangle \in C \\
\wedge&v_s,v_t\in\mathbb{V}\\
\wedge&v_s\Subset p_s \wedge v_t \Subset p_t \\
...
variables to ensure reduction for. However, as there is only a finite number of
variables in any given pattern, we can define these as separate cycles.

\begin{definition}\label{definition:variable-call-graph} Given a recursive call graph $G^r = \left\langle C^r, E^r \right\rangle$, let a ``unary recursive call graph'' $G^u= \left\langle C^u, E^u \right\rangle$, where
...
Visually we won't distinguish between variables, but rather, for any given call, we 

\begin{definition}\label{definition:increasing-decreasing-call} For \end{definition}

A size relation as defined in \referToDefinition{size-relation} is used to
deduce whether all the cycles in a call graph reduce a value.

\begin{lemma}\label{lemma:cycle-reduce} A call cycle $z=c_1,c_2,\ldots,c_n$ reduces a value on each
iteration iff $\forall c\in$ at least one of the participating control transitions reduces the
value and all other control transitions do not increase that value.\end{lemma}
...
We can analyze how a value changes its size through a call sequence by
...
A fail transition occurs if the values passed to a given clause to match its
