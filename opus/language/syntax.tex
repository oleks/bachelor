...
general. The term $\term{\_}$ in $\nonterm{pattern-value}$ is the conventional wildcard
operator; it indicates a value that won't used by the declaration, but allows
us to keep the same declaration signature. 

\begin{definition} A declaration signature in \D{} consists of the function
name and the number of parameters it has. We'll adopt the Erlang notation when
talking about function signatures, i.e. if we have a function \mono{less} that
takes in two parameters, we'll refer to it as \mono{less/2}.\end{definition}
