...
instance Show ShapeEdge where
  show (ShapeEdge sourceSignature targetSignature sourceName targetName change) =
    targetSignature ++ (':' : targetName) ++ (' ' : show change) ++
      (' ' : sourceSignature) ++ (':' : sourceName)
...
data ShapeChange
  ...
  | UnknownChange
  deriving(Eq)
instance Show ShapeChange where
  show Less = "<"
  show LessOrEqual = "<="
  show UnknownChange = "_|_"
